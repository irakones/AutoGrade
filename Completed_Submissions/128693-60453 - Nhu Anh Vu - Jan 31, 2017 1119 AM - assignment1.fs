...
(* Student name: Nhu Anh VU, Id Number: 260670511 *) 
...
let rec memberof pair = 
    match pair with
    | fst, [] -> false
    | fst, (x::xs) -> if fst = x then true 
                      else memberof(fst, xs)
...
let rec common twolists =
  let twolists = (selsort(fst(twolists)), selsort(snd(twolists)))
  match twolists with
  | [], [] -> []
  | ([], x::xs) -> []
  | (x::xs, []) -> []
  | (xs), (y::ys) -> if memberof(y, xs) then y::(common(xs, ys))
                     else common(xs, ys)
