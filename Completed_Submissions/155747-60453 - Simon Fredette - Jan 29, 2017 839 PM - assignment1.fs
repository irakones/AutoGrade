...
(* Student name: Simon Fredette, Id Number: 260 677 409 *) (* Edit this line. *)
...
let rec selsort l =
    match l with
        | [] -> []
        | lst -> (findMax lst)::selsort(remove((findMax lst), lst))
...
let rec common twolists =
    match twolists with
        | ([], lst) -> []
        | (x::xs, lst) -> if (memberof (x, lst)) then x::common (xs, lst) else common (xs, lst)
...
let rec mergesort l = 
    match l with
        ...
        | n::ns -> 
            let (lst_a,lst_b) = split (n::ns)
            let (sorted_a,sorted_b) = (mergesort lst_a, mergesort lst_b)
