...
(* Student name: Byungwoo Kim, Id Number: 260621422 *)
...
let w_mean weights data =
  let multiplied = pairlists(weights,data) |> List.map(fun (a,b) -> a*b)
  (sumlist multiplied)/(sumlist weights)
...
let findMax l = 
  let rec helper(l,m) =
    match (l,m) with
      | ([],y) -> y
      | (x::xs,y) -> if x > y then helper(xs,x) else helper(xs,y)
  match l with
...
let rec selsort l = 
  let max = findMax(l)
  let rmvdlst = remove(max,l)
  match rmvdlst with
    | [] -> [max]
    | x::xs -> max::selsort(x::xs)
