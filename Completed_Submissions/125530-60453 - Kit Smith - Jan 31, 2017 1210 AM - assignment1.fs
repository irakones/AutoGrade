...
(* Student name: Kit G. Smith, Id Number: 260665865 *) (* Edit this line. *)
...
let w_mean weights data =
    ...
    let numerator = List.map (fun (x, y) -> x * y) paired
    sumlist(numerator)/sumlist(weights)
...
let rec common twolists =
    match twolists with
        |(a, y) -> 
            match a with
                |[] -> []
                |x::xs -> if memberof(x, y) then x::common(remove(x,xs), y) else common(remove(x,xs), y)
...
let rec mergesort l = 
  ...
  | n::ns -> let (right, left) = split(l) in merge(mergesort(left), mergesort(right))
