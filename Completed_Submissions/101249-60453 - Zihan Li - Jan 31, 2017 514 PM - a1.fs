...
(* Student name: Zihan Li, Id Number: 260610677 *) (* Edit this line. *)
...
let w_mean weights data =
        let l = pairlists(weights, data) in 
            ...
            | [] -> 0.0
            | x::xs -> 
                let product = l |> List.map(fun(s,p) -> s*p) in
...
let rec common twolists = 
    ...
    | (x,[]) -> []
    | ([],y) -> []
    | (x::xs, y) -> if (memberof (x, y)) then x::common(xs,remove(x, y))
                    else common(xs,y)
