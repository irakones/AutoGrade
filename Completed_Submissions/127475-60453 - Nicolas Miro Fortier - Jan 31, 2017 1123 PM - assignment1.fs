...
(* Student name: Nicolas Fortier, Id Number: 260681624 *) (* Edit this line. *)
...
let rec selsort l = 
    match l with 
     | [] -> []
     | head :: tail -> let maximum = findMax l
...
let rec merge twolists = 
   ...
   | (p::ps,[]) -> p::ps
   | ([],y::ys) -> y::ys
   | (p::ps,y::ys) -> if p < y then p :: merge (ps, y::ys) else y:: merge(p::ps, ys)
