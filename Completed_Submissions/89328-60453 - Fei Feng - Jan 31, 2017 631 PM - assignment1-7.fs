...
(* Student name: Faye Feng, Id Number: 260575129 *) (* Edit this line. *)
...
let rec remove(item, lst) = 
    match (item, lst) with
    | (n,[]) -> []
    | (n,(x::xs)) -> if (n = x) then remove(n,xs) else x::(remove(n,xs))
...
let rec common twolists = 
    ...
    | ([],l) -> []
    | (l,[]) -> []
    | ((x::xs),l) -> if memberof(x,l) then x::(common(xs,remove(x,l))) else common(xs,l)
